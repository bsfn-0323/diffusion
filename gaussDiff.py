...
for i,T in enumerate(Ts):
    ...
    W = gaussScore(x0[idx],temp,nSteps,dt)
    xT = np.sqrt(temp)*torch.randn((P,N))
    x_recon = backward(xT,W,temp,nSteps,dt,True).numpy()
    np.save(f"data_N{N}_T{temp:.3f}_P{P}/recon_temp{T:.3f}_nSteps{nSteps}_dt{dt:.2f}",x_recon[:,slices,:])
    Dkls = np.append(Dkls,Dkl(x0[idx],x_recon[:,0,:]))
    del x_recon
    #print(f"-----Done {i}-----")
